"""
Utopia v2 — Brook Farm Agent Society Simulation

Based on v1. Three-phase rounds (Act / Talk / Reflect), structured agent
...
1. MORALE RESPONDS TO POSITIVE ACTIONS
   v1: Only ORGANIZE (+5) raised morale. Food crises drained -10 to -20/round.
       Result: morale hit 0 by round 10 in every run and never recovered.
   v2: FARM +2, TEACH +2, ORGANIZE +5, BUILD/REPAIR +1, TRADE +1 morale.
       Morale can stabilize at low levels instead of permanent flatline.

2. PER-AGENT PASSION BONUSES
   v1: All agents got identical satisfaction per action (WRITE +8, TEACH +5, etc).
       Hobbies couldn't anchor agents against morale collapse.
   v2: Each persona has a 'passions' field — 1-2 actions that give +3 to +5
       extra satisfaction. Dwight gets +5 for ORGANIZE (concerts), Sophia +3
       for TEACH, etc. Structurally justified: people have sustaining activities.

3. NEUTRAL REFLECTION PROMPT
   v1: Prompt said "Most days should be -5 to +3" — biased LLM toward
       negative satisfaction deltas. Agents spiraled to 0 in ~10 rounds.
   v2: Removed the bias. Prompt says "Negative if discouraging, positive if
       genuinely good. 0 if ordinary." LLM decides distribution from events.

4. SPEECH FATIGUE
   v1: SPEAK had no cost. Dana gave 15-22 speeches per run (30-73% of actions).
       The speech spiral dominated 4 of 6 runs.
   v2: Track consecutive SPEAK actions. After 3+, other agents' trust toward
       the speaker drops by 1, and environment notes the pattern. Social
       pressure against pure speechifying, without mechanically preventing it.

5. SOFTER FOOD ECONOMY
   v1: 5 food consumed per agent per round, FARM +8. Food crisis by round 5-6.
       No golden period — survival pressure dominated from the start.
   v2: 4 food consumed per agent per round. Food crisis delayed to ~round 10-12.
       Agents get a golden period to establish relationships and beliefs before
       survival pressures dominate. Matches Brook Farm's actual 2-year honeymoon.

6. EVENT-CATALYZED PROPOSALS
   v1: Historical events (Brisbane visit, phalanstery proposal) appeared as
       bulletin items. Agents never proposed systemic change — too busy surviving.
   v2: After ideological events, the next round's action prompt adds: "In light
       of [event], you may also consider proposing structural changes." Doesn't
       force proposals — makes them salient.
"""
...
def resolve_action(agent_name: str, decision: dict, env: Environment, agent: Agent) -> dict:
    ...
    if action.startswith("FARM"):
        ...
        env.morale = min(100, env.morale + 2)  # v2: visible effort lifts morale
        effects = {"food": 8, "satisfaction": sat_delta, "morale": 2}
        desc = f"{agent_name} spent the day working the fields. (+8 food, +2 morale)"
        ...
        env.morale = min(100, env.morale + 2)  # v2: school functioning lifts morale
        effects = {"money": 10, "satisfaction": sat_delta, "morale": 2}
        desc = f"{agent_name} taught classes at the school. (+$10, +2 morale)"
        ...
        if env.money >= 30:
            ...
            agent.satisfaction = min(100, agent.satisfaction + sat_delta)
            env.morale = min(100, env.morale + 1)  # v2: visible improvement
            effects = {"money": -30, "satisfaction": sat_delta, "morale": 1}
        ...
        agent.satisfaction = min(100, agent.satisfaction + sat_delta)
        effects = {"morale": 5, "satisfaction": sat_delta}
        ...
        agent.satisfaction = min(100, agent.satisfaction + sat_delta)
        env.morale = min(100, env.morale + 1)  # v2: engaging with outside
        effects = {"money": 15, "satisfaction": sat_delta, "morale": 1}
        ...
        if env.repair_building(building):
            ...
            agent.satisfaction = min(100, agent.satisfaction + sat_delta)
            env.morale = min(100, env.morale + 1)  # v2: visible improvement
            effects = {"money": -10, "satisfaction": sat_delta, "morale": 1}
...
def run_simulation(config: dict = None):
    ...
    run_dir = os.path.join(LOG_DIR, f"v2_{exp_name}_{timestamp}")
    ...
    print(f"\n{'='*60}")
    print(f"  BROOK FARM SIMULATION v2 — {exp_name}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Brook Farm v2 simulation")
