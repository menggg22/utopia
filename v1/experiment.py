"""
Experiment runner — run one or many simulation configs sequentially.

Usage:
    python experiment.py experiments/baseline_seed0.json           # run one
    python experiment.py experiments/baseline_seed*.json           # run several
    python experiment.py experiments/                              # run all in dir
"""
...
import glob
import time
from simulate_v1 import run_simulation
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python experiment.py <config.json or directory> [config2.json ...]")
        sys.exit(1)

    configs = []
    for arg in sys.argv[1:]:
        configs.extend(load_configs(arg))
